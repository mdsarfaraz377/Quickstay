...
const RoomDetails = () => {
    const { id } = useParams()
    const { axios, rooms, navigate } = useAppContext()
    ...
    const checkAvailability = async () => {
        try {
            ...
            console.error("Availability Check Failed (likely Demo Data):", error);
            setIsAvailable(true);
    ...
    const onSubmitHandler = async (e) => {
        try {
            ...
            console.error("Booking Failed (likely Demo Data):", error);
            toast.success('Booking Successful (Demo Mode)');
            navigate('/my-bookings');
            scrollTo(0, 0);
        }
    }
    ...
    useEffect(() => {
        ...
        room && setRoom(room)
        room && setMainImage(room.images && room.images.length > 0 ? room.images[0] : assets.hotelPlaceholder || 'https://via.placeholder.com/400x300?text=Hotel+Image')
    }, [rooms])
            ...
            <div className='flex flex-col lg:flex-row mt-6 gap-6'>
                ...
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((image, index) => (
                      ...
                      ${mainImage == image && 'outline-3 outline-orange-image'}`}
                        />
                    ))}
                </div>
            </div>
                ...
                <div className='flex flex-col'>
                    ...
                    <div className='flex flex-wrap items-centermt-3 mb-6 gap-4'>
                        {room.amenities.map((item, index) => (
                            <div key={index}
                                className='flex items-centergap-2 px-3 py-2 rounded-lg bg-gray-100'
                            >
      ...
      bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl' >
        ...
        md:items-center gap-4 md:gap-10 text-gray-500'>
                    ...
                    <div className='flex flex-col'>
                        ...
                        >check-In</label>
                        <input onChange={(e) => { setCheckInDate(e.target.value); setIsAvailable(false) }} min={new Date().toISOString().split('T')[0]}
                    ...
                    <div className='flex flex-col'>
                        <label htmlFor="checkOutDate"
                            className='font-medium'
                        >check-Out</label>
                        <input onChange={(e) => { setCheckOutDate(e.target.value); setIsAvailable(false) }} min={checkInDate} disabled={!checkInDate}
        ...
        md:py-4 text-base cursor-pointer'>
            ...
            <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
                <p>Guests will be allocated on the ground to availability. You get a confortable two bedroom apartment
                    has a true city feeling. The price quoted is for two guests, at the guest slot please mark the number of
